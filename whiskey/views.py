...
'''def all_product(request):
    ip = request.META['REMOTE_ADDR'] # поміщається конкретний юзер
    ...
    if current is None:
        cache.set(key_user, 1, timeout=period)
    elif int(current) >= limit:
        return JsonResponse({'Error':'Good luck'}, status = 429)
    else:
        cache.incr(key_user)
    
    pipe = r.pipeline()                        # метод для роботи з іншими фукнціями редіса
    pipe.incr(key_user,1)                      # інкремент
    pipe.expire(key_user,period)               # якщо період пройде, то буде оновлюватись
    pipe.execute()                         # опрацювання
